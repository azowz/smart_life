...
- 🎯 Entry points to execute your program using `python -m <final_project>` or `$ final_project` with basic CLI argument parsing.
...
from source
```bash
git clone hhttps://github.com/Su03l/final_project.git final_project
cd final_project
make install
```
...
```bash
$ final_project run --port 8080
```

or

```bash
python -m final_project run --port 8080
```

or

```bash
$ uvicorn final_project:app
```
...
```bash
❯ final_project --help
Usage: final_project [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ final_project create-user --help
Usage: final_project create-user [OPTIONS] USERNAME PASSWORD
...
```bash
final_project create-user admin admin --superuser
```
...
Run with `final_project run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `final_project create-user`.
...
-----------------------------------------------------
final_project/__init__.py              4      0   100%
final_project/app.py                  16      1    94%
final_project/cli.py                  21      0   100%
final_project/config.py                5      0   100%
final_project/db.py                   10      0   100%
final_project/models/__init__.py       0      0   100%
final_project/models/content.py       47      1    98%
final_project/routes/__init__.py      11      0   100%
final_project/routes/content.py       52     25    52%
final_project/routes/security.py      15      1    93%
final_project/routes/user.py          52     26    50%
final_project/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in final_project/default.toml.

### As environment variables
```bash
export final_project_KEY=value
export final_project_KEY="@int 42"
export final_project_KEY="@jinja {{ this.db.uri }}"
export final_project_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
final_project_ENV=production final_project run
```
